...
if CoreGui:FindFirstChild("__OG8_UI") then
    CoreGui.__OG8_UI:Destroy()
end
local oldBlur = Lighting:FindFirstChild("__OG8_BLUR")
...
local tagTxt = mkLabel(tagFrame, "v8.0 ULTRA", 11, Enum.Font.GothamBlack, Color3.fromRGB(8, 8, 14), Enum.TextXAlignment.Center)
...
local winSub = mkLabel(header, "Ultra Edition v8.0  —  Auto Farm Suite", 11, Enum.Font.GothamMedium, Color3.fromRGB(105, 112, 150))
...
local structKeywords = {
    ...
    "tesla", "constitution", "stillwater"
}

local bondKeywords = {
    "bond", "treasury", "bonus", "reward"
}
...
local function collectBondsAtPosition(position, radius)
    local collected = 0
    local searchRadius = radius or 80

    for _, container in ipairs({Items, Workspace}) do
        pcall(function()
            for _, item in ipairs(container:GetDescendants()) do
                if item and item.Parent then
                    ...
                    if n == "bond" or n == "bondpickup" or n == "bond_item" or n:find("bond") or n:find("treasury") then
                        isBond = true
                    end
                    ...
                    if isBond then
                        ...
                        if part and (part.Position - position).Magnitude <= searchRadius then
                            for _ = 1, 5 do
...
local function doBondsRun()
    ...
    if #structures == 0 then
        ...
        if rails then
            ...
            while G.running and track do
                ...
                if guide then
                    ...
                    task.wait(0.2)
                    collectBondsAtPosition(pos, 100)
                end
        ...
        for i, struct in ipairs(structures) do
            ...
            setStatus(string.format("[%d/%d] Teleportando para %s...", i, #structures, struct.model.Name), Color3.fromRGB(255, 190, 40))

            HRP.Anchored = true

            local targetPos = struct.position + Vector3.new(0, 8, 0)
            local steps = Settings.fastMode and 3 or 8

            for step = 1, steps do
                if not G.running then break end
                local alpha = step / steps
                HRP.CFrame = CFrame.new(HRP.Position:Lerp(targetPos, alpha))
                task.wait(0.02)
            end
            ...
            local collected = collectBondsAtPosition(struct.position, 120)

            for _, desc in ipairs(struct.model:GetDescendants()) do
                if desc:IsA("BasePart") then
                    local n = desc.Name:lower()
                    if n:find("bond") or n:find("chest") or n:find("loot") or n:find("pickup") or n:find("vault") then
                        pcall(function()
                            desc.CFrame = HRP.CFrame + Vector3.new(0, 3, 0)
                            if CollectRemote then
                                CollectRemote:FireServer(desc.Parent or desc)
                            end
                            local pp = desc:FindFirstChildWhichIsA("ProximityPrompt")
                            if pp then
                                pp.HoldDuration = 0
                                fireproximityprompt(pp)
                            end
            ...
            task.wait(Settings.fastMode and 0.3 or 0.6)

            local finalCollect = collectBondsAtPosition(struct.position, 120)
            collected = collected + finalCollect

            if collected > 0 then
                setStatus(string.format("[%d/%d] %s: %d bonds coletados!", i, #structures, struct.model.Name, collected), Color3.fromRGB(0, 230, 100))
                G.bonds = G.bonds + collected
...
local function doWinRun(skipBonds)
    ...
    if not skipBonds then
        ...
        for i = 1, maxStructures do
            ...
            if struct.hasBonds then
                setStatus(string.format("Pré-farm: teleportando para %s...", struct.model.Name), Color3.fromRGB(145, 55, 255))

                HRP.Anchored = true
                HRP.CFrame = CFrame.new(struct.position + Vector3.new(0, 8, 0))
                task.wait(0.3)

                local collected = collectBondsAtPosition(struct.position, 100)
                if collected > 0 then
                    G.bonds = G.bonds + collected
                    for _, l in ipairs(statRefs.bonds) do l.Text = tostring(G.bonds) end
                end
    ...
    setStatus("Teleportando para 80km (zona final)...", Color3.fromRGB(0, 210, 95))
    HRP.Anchored = true
    ...
    for i, cf in ipairs(WIN_POS) do
        ...
        setStatus(string.format("Avançando para alavanca... (%d/%d)", i, #WIN_POS), Color3.fromRGB(0, 210, 95))
        task.wait(Settings.fastMode and 0.8 or 1.2)
    end

    collectBondsAtPosition(HRP.Position, 150)
    task.wait(0.3)
    ...
    while G.running and not lever and tries < 100 do
        ...
        if tries % 10 == 0 then
            ...
            setStatus(string.format("Procurando alavanca... (tentativa %d)", tries), Color3.fromRGB(0, 200, 85))
        end
    ...
    if lever then
        ...
        if activated then
            setStatus("Alavanca ativada! Aguardando vitória...", Color3.fromRGB(0, 255, 130))
        else
            setStatus("Erro ao ativar alavanca! Tentando alternativo...", Color3.fromRGB(255, 140, 0))
            for _ = 1, 5 do
