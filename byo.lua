...
local tagTxt = mkLabel(tagFrame, "v9.3 FIXED", 11, Enum.Font.GothamBlack, Color3.fromRGB(8, 8, 14), Enum.TextXAlignment.Center)
...
local winSub = mkLabel(header, "Ultra Edition v9.3 Fixed  —  Auto Farm Suite", 11, Enum.Font.GothamMedium, Color3.fromRGB(105, 112, 150))
...
-- =====================================================
-- SISTEMA DE DETECÇÃO DE ESTRUTURAS CORRIGIDO
-- =====================================================
...
-- Detecta TODAS as estruturas do Workspace
local function scanAllStructures()
    local structures = {}
    local seen = {}
    ...
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") then
            ...
            if pos and not seen[obj] then
                seen[obj] = true
                
                -- BANCOS (vermelhos, com vault)
                if name:find("bank") and not name:find("combo") and not name:find("note") then
                ...
                -- FORT CONSTITUTION
                elseif name:find("fort") or name:find("constitution") or name:find("prescott") then
                    table.insert(structures, {
                        model = obj,
                        position = pos,
                        name = obj.Name,
                ...
                -- STERLING
                elseif name:find("sterling") then
                    table.insert(structures, {
                        model = obj,
                        position = pos,
                        name = obj.Name,
                ...
                -- MINAS
                elseif name:find("mine") and not name:find("cart") then
                    table.insert(structures, {
                        model = obj,
                        position = pos,
                        name = obj.Name,
                ...
                -- STILLWATER PRISON
                elseif name:find("prison") or name:find("stillwater") or name:find("excalibur") then
                    table.insert(structures, {
                        model = obj,
                        position = pos,
                        name = obj.Name,
    ...
    -- Ordena por posição Z
    table.sort(structures, function(a, b) return a.z < b.z end)
    return structures
end

-- =====================================================
-- COLETA DE BONDS - VÁRIAS TENTATIVAS
-- =====================================================
local function collectAllBonds()
    local collected = 0
    
    -- Procura em todo Workspace e RuntimeItems
    local containers = {Workspace, Items}
        ...
        pcall(function()
            for _, item in ipairs(container:GetDescendants()) do
                if item and item.Parent then
                    ...
                    if n == "bond" or n == "bonds" or n:find("treasury") or n:find("bonus") then
                        local part = nil
                        ...
                        if part then
                            ...
                            if dist <= 400 then
                                -- Puxa o bond para o jogador várias vezes
                                for i = 1, 15 do
                                    part.CFrame = HRP.CFrame + Vector3.new(math.random(-5, 5), 5, math.random(-5, 5))
...
-- =====================================================
-- ENTRAR NO VAULT/COFRE DOS BANCOS
-- =====================================================
local function enterBankVault(bankModel)
    -- Procura o vault door dentro do banco
    local vaultDoor = nil
    local vaultPosition = nil
    
    for _, child in ipairs(bankModel:GetDescendants()) do
        local name = child.Name:lower()
        if name:find("vault") or name:find("door") or name:find("safe") then
            if child:IsA("BasePart") then
                vaultDoor = child
                vaultPosition = child.Position
                break
            end
        end
    end
    
    -- Se não achou o vault, usa a posição do banco + offset
    if not vaultPosition then
        local bankPos = getModelPosition(bankModel)
        if bankPos then
            vaultPosition = bankPos + Vector3.new(0, 5, 0)
        end
    end
    ...
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("ProximityPrompt") then
            ...
            if parent then
                ...
                -- Prioriza alavancas na posição final
                if partPos and partPos.Z < -50000 then
                    score = score + 50
                end
...
local function doBondsRun()
    ...
    -- Escaneia TODAS as estruturas
    local structures = scanAllStructures()
    G.structuresFound = #structures

    if #structures == 0 then
        setStatus("Nenhuma estrutura encontrada!", Color3.fromRGB(255, 140, 0))
        task.wait(2)
    ...
    setStatus(string.format("%d estruturas encontradas!", #structures), Color3.fromRGB(255, 170, 0))
    
    -- Debug: mostra estruturas encontradas
    for i, s in ipairs(structures) do
        print(string.format("[%d] %s (tipo: %s) em Z=%.0f", i, s.name, s.type, s.z))
    end
    
    task.wait(0.5)

    -- Vai para cada estrutura
    for i, struct in ipairs(structures) do
        if not G.running then break end

        G.currentStructure = struct.name
        setStatus(string.format("[%d/%d] Indo para %s (%s)...", i, #structures, struct.name, struct.type), Color3.fromRGB(255, 190, 40))

        HRP.Anchored = true
        
        -- Determina a posição de destino baseada no tipo
        local targetPos = struct.position + Vector3.new(0, 25, 0)
        
        -- Se for banco, tenta entrar no vault
        if struct.type == "bank" then
            local vaultPos = enterBankVault(struct.model)
            if vaultPos then
                targetPos = vaultPos + Vector3.new(0, 3, 0)
            end
        end
        ...
        local steps = math.max(10, math.floor(distance / 100))
...
-- =====================================================
-- AUTO WIN CORRIGIDO - PERCORRE TRILHOS
-- =====================================================
local function flyToEndOfRails()
    ...
    -- O final do mapa é aproximadamente Z = -50000 a -55000 (baseado na experiência)
    -- Vamos usar -52000 como alvo e depois procurar a alavanca
    local targetZ = -52000
    ...
    local stepSize = Settings.fastMode and 500 or 250
    local steps = 0
    ...
    while currentZ > targetZ and G.running do
        ...
        -- Mantém X consistente (posição dos trilhos) e Y um pouco acima
        HRP.CFrame = CFrame.new(-424, 35, currentZ)
        
        -- Atualiza status a cada 100 passos
        if steps % 100 == 0 then
            local progress = math.min(100, math.floor((math.abs(startZ - currentZ) / math.abs(startZ - targetZ)) * 100))
            setStatus(string.format("Percorrendo... %d%% (Z: %.0f)", progress, currentZ), Color3.fromRGB(0, 210, 95))
        end
        
        task.wait(Settings.fastMode and 0.001 or 0.002)
    end
    
    -- Garante que chegou no final
    HRP.CFrame = CFrame.new(-424, 35, targetZ)
    setStatus("Chegou ao final! Procurando alavanca...", Color3.fromRGB(0, 230, 110))
...
local function doWinRun(skipBonds)
    ...
    if not skipBonds then
        ...
        -- Pega apenas as primeiras estruturas (antes dos 40km)
        for i, struct in ipairs(structures) do
            if not G.running then break end
            
            -- Só vai se estiver antes dos 40km (Z > -40000)
            if struct.z > -40000 then
    ...
    -- PERCORRE OS TRILHOS ATÉ O FINAL
    flyToEndOfRails()
    ...
    while G.running and not lever and tries < 300 do
        ...
        if tries % 30 == 0 then
            ...
            HRP.CFrame = CFrame.new(-424 + offset, 35, -52000)
    ...
    if lever then
        ...
        if lPos then
            -- Aproximação gradual
            HRP.CFrame = CFrame.new(lPos + Vector3.new(0, 10, 12))
            task.wait(0.3)
            HRP.CFrame = CFrame.new(lPos + Vector3.new(0, 6, 8))
            task.wait(0.3)
            HRP.CFrame = CFrame.new(lPos + Vector3.new(0, 4, 5))
            task.wait(0.4)
        end
        ...
        if activated then
            ...
            if lPos then
                HRP.CFrame = CFrame.new(lPos + Vector3.new(-12, 12, -8))
                task.wait(0.3)
            end
        else
