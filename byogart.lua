...
local tagTxt = mkLabel(tagFrame, "v9.1 FIXED", 11, Enum.Font.GothamBlack, Color3.fromRGB(8, 8, 14), Enum.TextXAlignment.Center)
...
local winSub = mkLabel(header, "Ultra Edition v9.1 Fixed  —  Auto Farm Suite", 11, Enum.Font.GothamMedium, Color3.fromRGB(105, 112, 150))
...
modeCard("bonds", "AUTO BONDS", "Percorre trilhos e coleta bonds das estruturas", "💰")
modeCard("win", "AUTO WIN", "Voa pelos trilhos até 80km e ativa a alavanca", "🏆")
modeCard("both", "AUTO WIN + BONDS", "Farm completo nos trilhos depois vitória", "⚡")
...
-- =====================================================
-- ESTRUTURAS QUE TÊM BONDS (ATUALIZADO)
-- =====================================================
local BOND_STRUCTURES = {
    -- Bancos (sempre têm bonds no cofre)
    "bank", "sterling", "vault",
    
    -- Pontos de Interesse principais
    "castle", "fort", "constitution", "prescott",
    "tesla", "lab", 
    "stillwater", "prison", "excalibur",
    "mines", "mine", "headframe",
    
    -- Cidade final (Outlaw's Town)
    "outlaw", "bandit", "final", "bridge", "drawbridge",
    
    -- Cidades fortificadas e towns
    "town", "fortified", "settlement",
    
    -- Lojas específicas que podem ter bonds
    "gunsmith", "doctor", "sheriff", "general", "store", "trading"
}

local function hasBonds(model)
    local name = model.Name:lower()
    for _, kw in ipairs(BOND_STRUCTURES) do
...
-- =====================================================
-- SISTEMA DE TRILHOS (RAILS) - CORRIGIDO
-- =====================================================
local function getAllRailSegments()
    local rails = {}
    
    -- Procura por RailSegments no Workspace
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("BasePart") or obj:IsA("Model") then
            local name = obj.Name:lower()
            -- Detecta segmentos de trilho
    ...
    -- Ordena por posição Z (do início ao fim)
    table.sort(rails, function(a, b) return a.z < b.z end)
    return rails
end

local function getStructuresAlongRails()
    local structures = {}
    local seen = {}
    
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") and hasBonds(obj) then
    ...
    -- Ordena por posição Z (distância percorrida)
                ...
                if item and item.Parent then
                    ...
                    if isBond then
                        ...
                        if part then
                            ...
                            if dist <= 250 then
                                for i = 1, 10 do
                                    part.CFrame = HRP.CFrame + Vector3.new(math.random(-3, 3), 3, math.random(-3, 3))
...
local function findLever()
    local leverKeywords = {"lever", "crank", "drawbridge", "bridge", "winch", "alavanca", "handle", "pull", "gate", "switch", "activate"}
    local bestLever = nil
    local bestPart = nil
    local bestScore = 0
        ...
        if obj:IsA("ProximityPrompt") then
            ...
            if parent then
                ...
                for _, kw in ipairs(leverKeywords) do
                    if pName:find(kw) then score = score + 15 end
                    if gpName:find(kw) then score = score + 8 end
                end
                ...
                -- Prioriza alavancas na posição final (80km = -80000 aprox)
                if partPos and partPos.Z < -75000 then
                    score = score + 30
                end
...
-- =====================================================
-- AUTO BONDS CORRIGIDO - PERCORRE TRILHOS E COLETA
-- =====================================================
local function doBondsRun()
    ...
    -- Obtém estruturas com bonds ordenadas por posição
    local structures = getStructuresAlongRails()
    ...
    setStatus(string.format("Encontradas %d estruturas com bonds!", #structures), Color3.fromRGB(255, 170, 0))
    task.wait(0.5)

    -- Percorre as estruturas em ordem (simulando percurso nos trilhos)
    for i, struct in ipairs(structures) do
        if not G.running then break end

        G.currentStructure = struct.name
        setStatus(string.format("[%d/%d] Voando para %s (Z: %.0f)...", i, #structures, struct.name, struct.z), Color3.fromRGB(255, 190, 40))

        -- Voa até a estrutura em velocidade extrema
        HRP.Anchored = true
        
        -- Movimento suave até a estrutura
        local targetPos = struct.position + Vector3.new(0, 15, 0)
        local startPos = HRP.Position
        local distance = (targetPos - startPos).Magnitude
        local steps = math.max(5, math.floor(distance / 200))
        ...
        if totalCollected > 0 then
            ...
            setStatus(string.format("[%d/%d] %s: verificando...", i, #structures, struct.name), Color3.fromRGB(150, 150, 150))
        end
...
-- =====================================================
-- AUTO WIN CORRIGIDO - VOA PELOS TRILHOS ATÉ 80KM
-- =====================================================
local function flyAlongRailsToEnd()
    setStatus("Iniciando voo pelos trilhos até 80km...", Color3.fromRGB(0, 210, 95))
    
    -- Obtém todos os segmentos de trilho ordenados
    local rails = getAllRailSegments()
    
    if #rails == 0 then
        setStatus("Trilhos não encontrados! Tentando método alternativo...", Color3.fromRGB(255, 140, 0))
        -- Fallback: vai direto para posições conhecidas do final
        return false
    end
    
    setStatus(string.format("%d segmentos de trilho encontrados!", #rails), Color3.fromRGB(0, 200, 85))
    
    HRP.Anchored = true
    
    -- Voa por cada segmento em velocidade extrema
    local stepSize = Settings.fastMode and 10 or 5
    for i = 1, #rails, stepSize do
        if not G.running then break end
        
        local rail = rails[i]
        HRP.CFrame = CFrame.new(rail.position + Vector3.new(0, 10, 0))
        
        -- Atualiza status a cada 100 segmentos
        if i % 100 == 0 then
            local progress = math.min(100, math.floor((i / #rails) * 100))
            setStatus(string.format("Percorrendo trilhos... %d%% (Z: %.0f)", progress, rail.z), Color3.fromRGB(0, 210, 95))
        end
        
        task.wait(Settings.fastMode and 0.005 or 0.01)
    end
    
    -- Garante que chegou no final
    local lastRail = rails[#rails]
    if lastRail then
        HRP.CFrame = CFrame.new(lastRail.position + Vector3.new(0, 10, 0))
        setStatus("Chegou ao final dos trilhos! Procurando alavanca...", Color3.fromRGB(0, 230, 110))
    end
...
local function doWinRun(skipBonds)
    ...
    if not skipBonds then
        setStatus("Win+Bonds: farmando estruturas primeiro...", Color3.fromRGB(145, 45, 255))
        setDetail("mode", "Win + Bonds")

        -- Primeiro coleta bonds das estruturas no caminho
        local structures = getStructuresAlongRails()
        local maxStructures = math.min(6, #structures)

        for i = 1, maxStructures do
            if not G.running then break end
            local struct = structures[i]
            
            setStatus(string.format("Pré-farm: indo para %s...", struct.name), Color3.fromRGB(145, 55, 255))
    ...
    -- VOA PELOS TRILHOS ATÉ O FINAL (sistema corrigido)
    setStatus("Voando pelos trilhos até 80km...", Color3.fromRGB(0, 210, 95))
    
    local railsSuccess = flyAlongRailsToEnd()
    
    -- Se não conseguiu pelos trilhos, usa posições de fallback
    if not railsSuccess then
        setStatus("Usando posições de fallback...", Color3.fromRGB(255, 170, 0))
        HRP.Anchored = true
        
        local fallbackPositions = {
            CFrame.new(-424, 28, -20000),
            CFrame.new(-424, 28, -40000),
            CFrame.new(-424, 28, -60000),
            CFrame.new(-424, 28, -75000),
            CFrame.new(-424, 28, -79000),
            CFrame.new(-424, 28, -80000),
        }
        
        for i, cf in ipairs(fallbackPositions) do
            if not G.running then break end
            HRP.CFrame = cf
            setStatus(string.format("Avançando... (%d/%d)", i, #fallbackPositions), Color3.fromRGB(0, 210, 95))
            task.wait(Settings.fastMode and 0.3 or 0.5)
        end
    end
    ...
    while G.running and not lever and tries < 150 do
        ...
        if tries % 20 == 0 then
            local searchOffset = (tries / 20) * 50
            HRP.CFrame = CFrame.new(-424 + searchOffset, 28, -80000)
    ...
    if lever then
        ...
        if lPos then
            -- Aproximação gradual
            HRP.CFrame = CFrame.new(lPos + Vector3.new(0, 5, 8))
            task.wait(0.3)
            HRP.CFrame = CFrame.new(lPos + Vector3.new(0, 3, 5))
            task.wait(0.3)
            HRP.CFrame = CFrame.new(lPos + Vector3.new(0, 2, 3))
            task.wait(0.4)
        end
        ...
        if activated then
            setStatus("Alavanca ativada! Posicionando para defesa...", Color3.fromRGB(0, 255, 130))
            
            if lPos then
                -- Posiciona em local seguro (sobre a ponte ou perto)
                HRP.CFrame = CFrame.new(lPos + Vector3.new(-8, 8, -5))
                task.wait(0.3)
            end
            
            setStatus("Aguardando vitória... (timer da alavanca)", Color3.fromRGB(0, 255, 130))
        else
        ...
        -- Tenta encontrar qualquer prompt na área
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("ProximityPrompt") then
    ...
    -- Aguarda o timer de vitória (4 minutos = 240 segundos)
    local waited = 0
    local waitTime = 250
    ...
    while G.running and waited < waitTime do
        ...
        -- Coleta bonds próximos durante a espera
        if waited % 10 == 0 then
            collectAllBonds()
        end
    end
